        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="group_cat_id" id="group_cat_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach($groupCats as $groupCat)
                            <option value="{{ $groupCat->id }}" {{ (old('group_cat_id', $user->group_cat_id) == $groupCat->id) ? 'selected' : '' }}>{{ $groupCat->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="education_background" id="education_background" value="{{ old('education_background', $user->education_background) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>

                <div class="flex items-start">
                    <div class="flex items-center h-5">
                        <input type="checkbox" name="work_status" id="work_status" value="1" {{ (old('work_status', $user->work_status) ? 'checked' : '') }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                    </div>
                ...
                <div id="work_details" class="{{ old('work_status', $user->work_status) ? '' : 'hidden' }} space-y-4">
                    <div>
                        <label for="job_title" class="block text-sm font-medium text-gray-700">Job Title</label>
                        <input type="text" name="job_title" id="job_title" value="{{ old('job_title', $user->job_title) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>

                    <div>
                        <label for="work_place" class="block text-sm font-medium text-gray-700">Work Place</label>
                        <input type="text" name="work_place" id="work_place" value="{{ old('work_place', $user->work_place) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <div class="bg-gray-50 p-4 rounded-md">
                    <div>
                        <input type="text" name="skills_input" id="skills_input" value=""
                        ...
                        <input type="hidden" name="skills_array" id="skills_array" value="{{ old('skills_array', json_encode($user->skills->pluck('name'))) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">Update User</button>
            </div>
        </form>
    </div>
</div>

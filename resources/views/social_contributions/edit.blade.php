...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <form method="POST" action="{{ route('social-contributions.update', $contribution->id) }}" class="bg-white p-6 rounded shadow space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $contribution->title) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary" required />
        ...
        <div class="pt-2">
            <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 transition ease-in-out duration-150">Save</button>
